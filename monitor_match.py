"""
Headless match monitor for the soccer simulation.

Runs the real GameEngine loop with a virtual clock (so a full 90s match
completes instantly), instruments shots/passes/possession, records
statistics and saves periodic screenshots for later inspection.
"""

import os
os.environ.setdefault("SDL_VIDEODRIVER", "dummy")
os.environ.setdefault("SDL_AUDIODRIVER", "dummy")

import math
import statistics

import pygame

# --- Virtual clock: patch pygame.time.get_ticks so the match can be
#     simulated at a fixed timestep without waiting in real time. -------------
_virtual_ms = {"t": 0}
_real_get_ticks = pygame.time.get_ticks


def _patched_get_ticks():
    return _virtual_ms["t"]


pygame.time.get_ticks = _patched_get_ticks

pygame.init()

from src.game_engine import GameEngine
from src.entities import Player, Ball

# --- Instrument shots / passes -------------------------------------------------
stats = {
    "shots": 0,
    "passes": 0,
    "pass_completed": 0,
    "kicks": 0,
    "possession_frames_t1": 0,
    "possession_frames_t2": 0,
    "free_ball_frames": 0,
    "goals": [],  # (time, scorer_team)
}

_orig_shoot = Player.shoot
_orig_pass = Player.pass_ball


def _shoot(self, ball, tx, ty):
    ok = _orig_shoot(self, ball, tx, ty)
    if ok:
        stats["shots"] += 1
    return ok


def _pass(self, ball, target):
    ok = _orig_pass(self, ball, target)
    if ok:
        stats["passes"] += 1
        stats["_pending_pass_target"] = target
    return ok


Player.shoot = _shoot
Player.pass_ball = _pass

FPS = 60
DT_MS = int(1000 / FPS)

engine = GameEngine()

# Track ball speed distribution, player spread (clustering), possession streaks.
ball_speeds = []
team_spread_samples = []  # avg pairwise distance within team1
min_pair_dist_samples = []  # closest two players (any) => clustering / overlap
frames = 0
prev_score = (0, 0)
last_possessor = None

os.makedirs("match_frames", exist_ok=True)

MATCH_FRAMES = FPS * engine.match_duration  # simulate whole match

for i in range(MATCH_FRAMES + 5):
    _virtual_ms["t"] += DT_MS
    engine.update()
    frames += 1

    ball = engine.ball
    speed = math.hypot(ball.vx, ball.vy)
    ball_speeds.append(speed)

    # possession accounting
    poss = ball.possession
    if poss is None:
        stats["free_ball_frames"] += 1
    elif poss in engine.team1.players:
        stats["possession_frames_t1"] += 1
    else:
        stats["possession_frames_t2"] += 1

    # detect completed pass (possession returns to a teammate after a pass)
    if "_pending_pass_target" in stats and poss is stats["_pending_pass_target"]:
        stats["pass_completed"] += 1
        del stats["_pending_pass_target"]

    # goal detection via score change
    score = (engine.team1_score, engine.team2_score)
    if score != prev_score:
        scorer = "Team 1" if score[0] > prev_score[0] else "Team 2"
        stats["goals"].append((round(engine.game_time, 1), scorer))
        prev_score = score

    # clustering metrics
    all_players = engine.team1.players + engine.team2.players
    t1 = engine.team1.players
    pair_d = [t1[a].distance_to(t1[b]) for a in range(len(t1)) for b in range(a + 1, len(t1))]
    if pair_d:
        team_spread_samples.append(statistics.mean(pair_d))
    all_pair = [all_players[a].distance_to(all_players[b])
                for a in range(len(all_players)) for b in range(a + 1, len(all_players))]
    if all_pair:
        min_pair_dist_samples.append(min(all_pair))

    # save screenshots at 5 evenly spaced moments
    if i in {int(MATCH_FRAMES * f) for f in (0.05, 0.25, 0.5, 0.75, 0.95)}:
        engine.render()
        pygame.image.save(engine.screen, f"match_frames/t_{int(engine.game_time)}s.png")

# --- Report --------------------------------------------------------------------
moving = [s for s in ball_speeds if s > 1]
print("=" * 60)
print("MATCH MONITOR REPORT")
print("=" * 60)
print(f"Simulated frames: {frames}  (~{engine.game_time:.1f}s match, {FPS} fps)")
print(f"Final score: Team 1 {engine.team1_score} - {engine.team2_score} Team 2")
print(f"Goals: {stats['goals']}")
print(f"Total goals: {len(stats['goals'])}  -> {len(stats['goals'])/max(engine.game_time,1)*60:.1f} goals/simulated-minute")
print("-" * 60)
print(f"Shots taken: {stats['shots']}")
print(f"Passes attempted: {stats['passes']}  completed(approx): {stats['pass_completed']}")
print("-" * 60)
tot = stats["possession_frames_t1"] + stats["possession_frames_t2"] + stats["free_ball_frames"]
print(f"Possession  T1: {stats['possession_frames_t1']/tot*100:5.1f}%  "
      f"T2: {stats['possession_frames_t2']/tot*100:5.1f}%  "
      f"Free ball: {stats['free_ball_frames']/tot*100:5.1f}%")
print("-" * 60)
print(f"Ball speed  max: {max(ball_speeds):.1f}  mean(moving): {statistics.mean(moving) if moving else 0:.1f}")
print(f"Frames ball essentially still (<1 px/s): {sum(1 for s in ball_speeds if s<=1)/len(ball_speeds)*100:.1f}%")
print("-" * 60)
print(f"Team1 avg intra-team spread: {statistics.mean(team_spread_samples):.1f} px")
print(f"Closest any-two players (mean of per-frame min): {statistics.mean(min_pair_dist_samples):.1f} px")
print(f"Frames with two players overlapping (<15px): "
      f"{sum(1 for d in min_pair_dist_samples if d<15)/len(min_pair_dist_samples)*100:.1f}%")
print("=" * 60)
