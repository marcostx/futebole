...
- [x] **Defensive auto-select with manual switch (required):** when Team 1 is
...
- [x] Selection follows the ball on attack: when Team 1 gains possession, snap
      ...
      - Fixed the PR #30/#31 Copilot finding here: replaced the roster-removal
        trick with an `AIController.controlled_player` exclusion — the human's
        player stays in `team.players` (so `update_team_state()` correctly
        reports `attack` and teammates support the attack), while the AI skips
        it in active/support selection, the movement loops, the carrier action
        block, and the keeper update.
- [x] On-ball actions for the selected player (gated by `Player.can_act()`
...
- [x] Selection indicator (`src/ui.py`): draw a distinct marker (e.g. a colored
...
- [ ] Goalkeeper policy: keep the GK AI-controlled for the human team by default
      ...
      the keeper.
- [x] Fairness: the human-controlled player goes through the same central
...
- [x] Controls & docs: keep SPACE (pause), R (reset), ESC (quit); add an
